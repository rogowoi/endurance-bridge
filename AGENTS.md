...
4. For Codex, register `https://endurance-bridge-api.vercel.app/api/mcp` using `ENDURANCE_BRIDGE_API_KEY` as the bearer-token environment variable.
...
6. Restart or reload the client if required. Verify tool discovery, call `garmin_connection_status`, and make one harmless read such as `garmin_list_schedules`.
