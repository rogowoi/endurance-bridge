...
## When someone wants to use Endurance Bridge
...
6. After adding the server, fully quit the Codex desktop app with **Codex → Quit Codex** or `⌘Q`, then reopen it. Closing a window is not enough. Start a clean task with “Review my training from last week” and verify one `endurance_get_period` read; do not lead with provider status. If startup says `ENDURANCE_BRIDGE_API_KEY` is not set, repeat the dashboard command and fully restart Codex again.
...
6. Create an MCP key, register the appropriate client, then verify the provider-neutral `endurance_*` tools and a live read.
...
- Keep the public MCP contract provider-neutral. Do not add public provider-prefixed tool aliases.
